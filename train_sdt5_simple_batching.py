...
# all ML stuff
from transformers import CLIPFeatureExtractor, FlaxT5EncoderModel, T5Tokenizer
...
def main(epoch=0, steps_offset=0, lr=2e-6):
    ...
    batch_processor = Thread(
        target=child_thread_wrapper, args=[batch_queue, batch_order, worker_count]
    )
    ...
    for x in batch_order:
        ...
        batch = shard(current_batch["numpy_data"])
