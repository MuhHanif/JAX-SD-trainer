...
def stream_data(
    ...
    if len(selected_df) % batch_size > 0:
        remainder_selected_df = selected_df.iloc[
            -(len(selected_df) % batch_size) :
        ]  # modulo return 0 bug!
    ...
    def _stream_process(df: pd.DataFrame):
        ...
        while len(streamed_data) == 0:
            ...
            image = stream_image_fn(
                url=image_url, threshold_size=min_res_axis, debug=debug
            )
            ...
            if image != None:
                ...
                df.iloc[counter, df.columns.get_loc(check_col)] = (
                    max_check_col_value + 1
                )
    ...
    for streamed_data in batch_image:
        ...
        image = process_image_fn(
            image=streamed_data["image"], rescale_size=streamed_data["size"]
        )
...
def worker_batching(batch_order: list, internal_queue_length: int) -> list:
    """
    Distributes a list of items into batches, each with a maximum length specified by the internal_queue_length parameter.

    Args:
        batch_order (list): The list of items to be batched.
        internal_queue_length (int): The maximum length of each batch.

    Returns:
        list: A list of batches, where each batch is a list of items from the original list.

    Example:
        >>> my_list = list(range(20))
        >>> batches = worker_batching(my_list, 3)
        >>> print(batches)
        [[0, 1, 2, 9, 10, 11, 18, 19], [3, 4, 5, 12, 13, 14], [6, 7, 8, 15, 16, 17]]
    """
    # create batch slot
    batch_order_pool = [[] for x in range(internal_queue_length)]
    queue_slot = 0

    for counter, x in enumerate(batch_order):
        # keep track of the queue length
        modulo_counter = counter % internal_queue_length

        batch_order_pool[queue_slot].append(x)
        # if intetrnal queue length is reached switch to other slots
        if modulo_counter < internal_queue_length - 1:
            continue

        queue_slot = queue_slot + 1
        if queue_slot == internal_queue_length:
            queue_slot = 0

    return batch_order_pool
