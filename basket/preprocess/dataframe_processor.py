...
def discrete_scale_to_equal_area_old(
    ...
    loc_boolean_map = (
        dataframe[clamped_height] / dataframe[clamped_width] >= aspect_ratio_clamping
    )
    dataframe.loc[loc_boolean_map, clamped_height] = (
        dataframe.loc[loc_boolean_map, clamped_width] * aspect_ratio_clamping
    )
    loc_boolean_map = (
        dataframe[clamped_width] / dataframe[clamped_height] >= aspect_ratio_clamping
    )
    dataframe.loc[loc_boolean_map, clamped_width] = (
        dataframe.loc[loc_boolean_map, clamped_height] * aspect_ratio_clamping
    )
    ...
    new_height = (
        (dataframe[clamped_height] * image_area) // nearest_multiple * nearest_multiple
    )
    new_width = (
        (dataframe[clamped_width] * image_area) // nearest_multiple * nearest_multiple
    )
    ...
    dataframe.loc[
        loc_boolean_map, [new_image_width_col_name, new_image_height_col_name]
    ] = maximum_area ** (1 / 2)
...
def scale_by_minimum_axis(
    ...
    new_width = (round(dataframe[image_width_col] * scale_factor) // 64 * 64).astype(
        int
    )
    new_height = (round(dataframe[image_height_col] * scale_factor) // 64 * 64).astype(
        int
    )
    ...
    dataframe[new_res_col] = dataframe[new_res_col].apply(
        np.clip, a_min=target_minimum_scale, a_max=target_maximum_scale
    )
    return dataframe
...
def resolution_bucketing_batch(
    ...
    for group, data in dataframe:
        ...
        tail_end_length = len(
            data.loc[data[bucket_group_col_name] == data[bucket_group_col_name].max()]
        )
    ...
    replace_dict = dict(
        zip(first_batch_order_array, list(range(len(first_batch_order_array))))
    )
